...
static void m_power_on_bar_1_event_handler (lv_event_t *e)
{
  ...
  if (code == MY_EVENT_POWER_ON) {
    ...
    lv_label_set_text(guider_ui.power_on_label_1, msg);
	  ...
	  // lv_obj_set_style_anim_time(guider_ui.power_on_bar_1, ANIM_INTERVAL, 0);
    lv_bar_set_value(guider_ui.power_on_bar_1, set_val, LV_ANIM_ON);
    ...
    if (anim_st == -1){
      ...
      if (val >= lv_bar_get_max_value(guider_ui.power_on_bar_1)){
        ...
        // 这里不能多次进入，否则会死机
        ui_load_scr_animation(&guider_ui, &guider_ui.home, guider_ui.home_del, &guider_ui.power_on_del, setup_scr_home, LV_SCR_LOAD_ANIM_OVER_RIGHT, 200, 200, true, true);
      }
    }
  }
}
void bar_show_2()
{
   ...
   lv_bar_set_value(bar,10,LV_ANIM_ON);  //设置初始值
}
...
void station_init2(lv_timer_t * tm)
{
  ...
  lv_timer_set_repeat_count(lv_timer_create(station_init3, ANIM_INTERVAL+10, NULL), 1);
}
void station_init1(lv_timer_t * tm)
{
  trigger_power_on_progress(PHASE_POWER_ON_WIFI_CONNECTING);
  lv_timer_set_repeat_count(lv_timer_create(station_init2, ANIM_INTERVAL+10, NULL), 1);
}
...
void custom_init(lv_ui *ui)
{
  ...
  // start trigger!
  lv_timer_set_repeat_count(lv_timer_create(station_init1, 1, NULL), 1);
}
